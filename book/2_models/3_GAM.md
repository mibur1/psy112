...
So in comparison to simple splines regression which ([last semester](https://mibur1.github.io/psy111/book/statistics/spline-reg/introduction)) was introduced to predict $y$ on the basis of a single predictor $x$, GAMs are a generalization which allows us to predict $y$ given multiple predictors $x_1 ... x_p$.
...
There are multiple options for implementing GAMs. We will here use `statsmodels`, as you should already be familiar with it from the previous semester. The workflow is the following:
...
- The `coef` column shows the estimated coefficients (the individual spline basis coefficients are not interpreted on their own)
...
Each subplot shows how the modelled relationship between a feature and the target behaves nonlinearly.

- `plot_se=True` adds a confidence band around the estimated smooth curve
- `cpr=True` overlays a component-plus-residual plot (the partial working residuals as a scatter)
